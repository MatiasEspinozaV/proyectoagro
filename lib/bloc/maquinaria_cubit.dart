...
class MaquinariaCubit extends Cubit<MaquinariaState> {
  ...
  Future<void> fetchMaquinarias(int page, int pageSize, {String search = ''}) async {
  emit(MaquinariaLoading());
  final url = Uri.parse('https://controller.agrochofa.cl/api/sgm/maquinarias/public');

  try {
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'page': page, 'pageSize': pageSize, 'search': search}),
    );
  ...
  void deleteMaquinaria(String id) {
    if (state is MaquinariaLoaded) {
      ...
      final updatedList = currentState.maquinarias.where((item) => item.id != id).toList();
      emit(MaquinariaLoaded(maquinarias: updatedList, total: updatedList.length));
    }
  }
}
